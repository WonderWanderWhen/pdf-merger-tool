...
def process_files(files, order, pdf_name):
    ...
    merged_text = ""   # To accumulate text from TXT, DOCX, XLSX

    # Process files in the order specified by the dropdown.
    # Here, 'order' is a list of strings like "0: filename"
    for o in order:
        try:
            # Extract the index from the string (before the colon)
            idx = int(o.split(":")[0])
        ...
        file_item = files[idx]
        # If file_item is a dict, extract its name and data
        if isinstance(file_item, dict):
            ...
            # Otherwise, assume it’s a file-like object with a .name attribute
    ...
    # Convert any merged text to a PDF page
    if merged_text:
        text_pdf = fitz.open()
    ...
    # Create a new PDF document and merge all PDFs
...
def reorder_files(files):
    ...
    for i, file_item in enumerate(files):
        # Handle both dict and object types for the uploaded file.
        if isinstance(file_item, dict):
            name = file_item.get("name", "")
        else:
            name = getattr(file_item, "name", "")
        choices.append(f"{i}: {str(name)}")
    # Return a Gradio update with these simple string choices.
